...
def plot_spending_chart(df):
    ...
    # 1️⃣ Category-wise Spending Breakdown
    ...
    # 2️⃣ Income vs Expense Comparison
    st.subheader("💰 Income vs Expense")
    income_expense = df.groupby("transaction_type")["amount"].sum()

    if not income_expense.empty:
        fig, ax = plt.subplots(figsize=(5, 2.5))
        # Ensure order Expense, Income
        income_expense = income_expense.reindex(["Expense", "Income"], fill_value=0)
        income_expense.plot(kind="bar", color=["red", "green"], ax=ax)
